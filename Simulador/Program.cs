...
namespace Simulador
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            for (int i = 1; i <= numeroEquipos; i++)
            {
                var gg = generadorJug.generaCuerpoTecnico(i);
                var dire = generadorJug.generaDirectiva(i);
                var equipoTotal = new EquipoTotalCO(listaEquiposVO[i - 1] as EquipoVO, generadorJug.generaPlantilla(i),
                    gg, dire, null);
